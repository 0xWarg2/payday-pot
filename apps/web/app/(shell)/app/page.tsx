...
import { PaydayReminderCard } from "@/components/dashboard/PaydayReminderCard";
...
export default function DashboardPage() {
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-[26px] font-semibold tracking-tight sm:text-[30px]">Your pool</h1>
        <p className="text-fg-muted mt-1 max-w-[62ch] text-[15px] leading-relaxed">
          Your balance, your weight and your winnings are encrypted on chain. The round itself is public.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        <Cell mobile="order-2" desktop="lg:order-1 lg:col-span-5">
          <PrivatePositionCard />
        </Cell>
        <Cell mobile="order-1" desktop="lg:order-2 lg:col-span-7">
          <NextDrawCard />
        </Cell>

        <Cell mobile="order-4" desktop="lg:order-3 lg:col-span-5">
          <TwabCard />
        </Cell>
        <Cell mobile="order-3" desktop="lg:order-4 lg:col-span-7">
          <QuickActions />
        </Cell>

        <Cell mobile="order-6" desktop="lg:order-5 lg:col-span-5">
          <PaydayReminderCard />
        </Cell>
        <Cell mobile="order-5" desktop="lg:order-6 lg:col-span-7">
          <EmployerBoostCard />
        </Cell>

        <Cell mobile="order-7" desktop="lg:order-7 lg:col-span-12">
          <PrivateActivityList />
        </Cell>
      </div>
    </div>
 ...
 */
function Cell({ mobile, desktop, children }: { mobile: string; desktop: string; children: ReactNode }) {
  return <div className={`${mobile} ${desktop}`}>{children}</div>;
}
