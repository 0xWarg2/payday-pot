...
| 9 | 27/08 | Sepolia RC live | ✅ 150 contract + 288 web unit + **79 e2e** (1 skip) | ✅ vòng draw thật, RUNBOOK §8 hai bảng tx | ✅ | ✅ **7/7 dòng exit gate đạt 03/09** — RC `0x792c77D9…a035` sống trên Sepolia (epoch **3600s**, prize 50 USDC employer-funded), source verified hai chỗ (Blockscout `#code` + Sourcify **full match**), public URL trả về app thật và cross-origin isolated (`check-deploy.sh` 5 route). **Dòng exit gate nặng nhất đo được trên chain:** epoch #2 cắt ngang giữa lượt scan (cursor 1/2) rồi **ví khác** — không keeper, không owner, chỉ có ETH — chạy tiếp tới Settled, không bước nào lặp, seed rút đúng một lần. **Bug thật:** `decryptWithRetry` chưa bao giờ có tác dụng (cùng body → cùng `requestId` → cùng câu trả lời hỏng); cái đổi được kết quả là **tập handle**, và đổi nó miễn phí vì không nằm trong payload EIP-712 → `decryptTargets` tách từng handle, mở được một phần, phần còn lại **ở lại ẩn** chứ không thành `0`. `kms-probe.ts` chạy `userDecrypt` trong Node đóng câu hỏi "lỗi của web hay của hạ tầng" — và lật luôn quirk #47. Cùng đó: 4 deployment fail trên Vercel với 4 nguyên nhân khác nhau, trong đó một deployment fail **vẫn trả HTTP 200**. Dòng nặng nhất trả lời bằng chính bộ test: `E2E_BASE_URL` trỏ cả suite vào **https://payday-pot-vuitinhvl7xs-projects.vercel.app** và không dựng webServer → **77 xanh / 3 skipped trên production**. `dev` → `main` merge `--no-ff` (`0593260`, mang cả Day 7–8–9), tag `rc-1`. Còn lại là deliverable của người: video, form, thread. Chi tiết `docs/handoffs/DAY_09_HANDOFF.md` |
| 10 | 28/08 | Polish + submit | ✅ 150 contract + **359 web unit** + **101 e2e** (6 skip) | ✅ `localhost:3001` 5 route, 0 console error | ✅ | ◐ **UI đợt 2 xong 04/09 (chưa commit, `dev`) + cắt chữ đợt 2 (landing 306→250 từ, `/app` 235→128) + hero text stagger** — ít chữ hơn trên landing / `/app` / Draw Room (mỗi section còn h2 + 1 câu + "Read the docs →"; `PaydayReminderCard` xoá; `PHASE_LABEL` hợp nhất về `lib/draw/room.ts`), mục **`/docs`** kiểu docs.pooltogether.com (7 trang, sidebar theo nhóm, TOC, prev/next, `#unwrap` giữ nguyên cho `PendingUnwrapBanner`, trang Contracts đọc thẳng manifest và có drift gate), Tier B depth (tilt 1 card · bento 12 cột có tầng · scroll reveal `view()` · hero coin 3D); reduced-motion đo được 0 animation. Chi tiết `docs/UI_DEPTH_DIRECTION.md`. **UI đợt 3 (04/09 sáng, chưa commit):** Geist qua `next/font` + thang chữ token; `fg-muted` nâng contrast; phòng tối bỏ tím, thêm `draw-warning`, cyan chỉ = encrypted; **bug thật** `ConfidentialValue` `text-fg` trên `draw-surface` = 1.05:1 (giá trị trong Draw Room gần vô hình) → prop `surface="draw"` + `contrast.test.ts` chặn tái phát; **lịch sử giao dịch đọc từ chain** (4 event không amount, chunk 40k, incremental, gộp với hàng local, không ghi thêm storage); Docs có **6 hình SVG** vẽ trong code + trang `/docs/get-started`; onboarding mỗi bước ≤1 câu + `Guide →`. Gate: **359 web unit** + **101 e2e** (6 skip, `history.spec.ts` mới) qua build production; console 0 lỗi; reduced-motion 0 animation. **UI đợt 4 (04/09 sáng, chưa commit):** chữ chuyển động "sinh động vừa" toàn app — `Words`/`Typed`/`InView`/`CountUp` (`components/motion/`), heading tách từ theo bậc, badge gõ ký tự, số public đếm lên khi vào view, link gạch tự vẽ; Draw Room bản êm, sealed card/progress không đổi; confidential không bao giờ đếm (guard test). Gate: **368 web unit** + **101 e2e** (6 skip) vào dev :3001; reduce ⇒ 0 animation; 0 console error. **Logo** = đồng xu 3D của hero vẽ SVG (`components/brand/CoinMark.tsx`, favicon `app/icon.svg`, apple-icon + OG image render từ cùng hình) gắn header app/docs/onboarding. **Vercel**: project `payday-pot` (root `apps/web`) đã nối GitHub, production branch `main` ⇒ merge `dev`→`main` tự deploy production; `dev` ra preview; CLI `vercel deploy --prod` từ root (RUNBOOK §12). Còn lại là deliverable của người: video, form, thread. |
