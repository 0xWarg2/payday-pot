...
## Trạng thái 29/08 (hết Day 8)

**14/15 dòng đóng. Còn đúng một dòng, và nó không thiếu code — nó thiếu tiền gas.**

| | |
|---|---|
| Đóng đủ 3 điều kiện (UI + action + test) | R2 R3 R4 R5 R6 R7 R8 R9 R10 R11 R12 R13 R14 R15 |
| Còn nợ | **R1** — chỉ thiếu nút *Resume finalize*. Detect, banner, idempotent re-check, taxonomy revert đều xong và có E2E; ẩn số duy nhất là nội dung `signatures` của `finalizeUnwrap`, và để gửi thử một lần cần **một unwrap treo thật trên ví có tiền** |

Đọc kèm: `docs/handoffs/DAY_08_HANDOFF.md`. Con số test của Day 8: **261** web unit ·
**70** e2e (6 skipped) · **150** contract.

## Bảng failure mode

| # | Failure mode | User thấy gì | Recovery action | Test | Ngày | ☐ |
|---|---|---|---|---|---|---|
| R1 | **Unwrap pending / `finalizeUnwrap` chưa chạy** (user cancel signature, tab đóng, RPC lỗi) | Banner bền vững "bạn có 1 unwrap chưa hoàn tất", nói rõ tiền đang nằm ở token contract, KHÔNG hiện balance = 0 | Nút **Resume finalize**, idempotent, gọi lại được nhiều lần | E2E: kill tab giữa unwrap → reload → resume thành công | 6 (onboarding) + 8 (E2E) | ◐ **thiết kế chốt 26/08** — detect: đọc `UnwrapRequested` (hoặc lưu requestId lúc ký) rồi `unwrapRequester(id) != address(0)` ⇒ còn treo; `== address(0)` ⇒ đã finalize. Một view call, không backend, sống qua reload/đổi máy. Recovery: `finalizeUnwrap(id, amount, proof)` **permissionless** — banner nói "ví nào cũng bấm được", đúng tinh thần R4. Idempotent kiểu **revert-not-corrupt**: bấm lần 2 thì requester đã là 0 → revert sạch, không mất tiền; UI bắt revert đó và chuyển banner sang "đã hoàn tất" thay vì hiện lỗi đỏ. Amount cho finalize lấy từ `unwrapAmount(id)` — public-decryptable theo thiết kế của wrapper (PRIVACY §2 mục 3), nên không cần chữ ký user thứ hai. Code Day 6 (onboarding) + E2E Day 8 · **Day 8 — probe read-only 29/08 (`eth_call`, không gas, không khoá) trên cUSDC live**: `finalizeUnwrap(bytes32,uint64,bytes)` **tồn tại đúng chữ ký đang pin trong `CUSDC_ABI`** (biến thể `bytes[]` của FHEVM oracle và biến thể `uint256` đều không có selector), và một requestId không tồn tại revert `InvalidUnwrapRequest(bytes32)` = `0xd1630f8e` — decode được, pin ở `test/classify-read-error.test.ts`. UI Day 6 shipped: banner nói ba câu (hai bước · bước hai chưa settle · **nothing is lost**), link explorer, link KNOWN_LIMITATIONS, nút **Check again** idempotent; E2E `e2e/recovery.spec.ts` 3 test — banner sống qua reload, bấm lại không nhân đôi, `unwrapRequester → address(0)` làm banner **biến mất chứ không đỏ**, và không ô confidential nào hiện chữ số. **CÒN NỢ ĐÚNG MỘT THỨ**: nút *Resume finalize* thật. Ẩn số duy nhất là nội dung tham số `signatures` — gần như chắc là `decryptionProof` của `publicDecrypt` (relayer-sdk 0.4.1 trả `{clearValues, abiEncodedClearValues, decryptionProof}`), nhưng chưa gửi được lần nào vì cần **một unwrap treo thật trên ví có tiền**. Ship nút chưa từng chạy đúng một lần = dead end, đúng thứ dòng này cấm. Unblock: cùng ví/gas với việc tay còn nợ của Day 7 |
